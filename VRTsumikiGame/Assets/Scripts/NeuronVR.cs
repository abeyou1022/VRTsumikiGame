...
public class NeuronVR : NeuronInstance
{
    ...
    public NeuronVR(string address, int port, int commandServerPort, NeuronConnection.SocketType socketType, int actorID)
        : base(address, port, commandServerPort, socketType, actorID)
    {
    }

    public NeuronVR(Animator animator, string address, int port, int commandServerPort, NeuronConnection.SocketType socketType, int actorID)
        : base(address, port, commandServerPort, socketType, actorID)
    {
    ...
    public NeuronVR(Animator animator, NeuronActor actor)
        : base(actor)
    {
        boundAnimator = animator;
        UpdateOffset();
    ...
    public NeuronVR(NeuronActor actor)
        : base(actor)
    {
    }
    ...
    public void ApplyMotion(NeuronActor actor, Animator animator, Vector3[] positionOffsets, Vector3[] rotationOffsets)
    {
        ...
        if (actor.withDisplacement)
        {
            ...
            {
                SetPosition(animator, HumanBodyBones.Head, MainCamera.position);
            }
        ...
        {
            Quaternion quaternion = MainCamera.rotation;
            SetRotation(animator, HumanBodyBones.Head, new Vector3(quaternion.eulerAngles.x, quaternion.eulerAngles.y, quaternion.eulerAngles.z));
        }
